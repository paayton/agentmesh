...
| 编号 | 角色 | 核心诉求 | 主要触点 |
|---|---|---|---|
| ① | 终端使用者（开发者/运营/排障） | 开箱即用、免配密钥（走中台 Proxy）、可选不同厂商模型、越用越顺手 | OpenClaw 客户端 |
...
|---|---|---|---|---|---|---|
| PRD-01 | 会话启动灰度绑定：client_id→GrowthBook→版本串→KitOps ref→pull 加载 | ① | ① | FR2/FR3 | M1 | 同一 client_id 多次启动稳定绑定同 variant；离线命中缓存跳过拉取；无缓存回退 baseline |
| PRD-02 | trace 带 variant 标签回灌 Langfuse | ①/⑤ | ② | FR7 | M1 | 每条 trace 含 variant/asset 版本维度；Langfuse 可按 variant 过滤 |
...
| PRD-11 | 技能沉淀可视化（本地 skill 清单 + 手动标记） | ① | ③ | FR4 | M3 | 客户端展示已沉淀 skill；用户可手动标记"值得沉淀" |
...
| ID | 需求 | 角色 | 环节 | FR | 里程碑 | 验收标准 |
|---|---|---|---|---|---|---|
| PRD-13 | 回流共享 + 去标识化预览 | ②/① | ⑥ | FR6 | M4 | 上传默认关闭；显式授权；上传前预览去标识化内容确认不含机密 |
...
| OpenHands 加载外部 ModelKit 机制未定 | 架构 §8 | M1 前实测确认加载点 |

## 8. 里程碑与需求映射

| 里程碑 | 目标 | 覆盖需求 |
|---|---|---|
| M0 环境就绪 | 平台开源件自托管可用、中台 LiteLLM Proxy 调通云端厂商 | 基础设施 |
| M1 MVP 竖切 | 验证 FR2+FR3+FR7 闭环成立 | PRD-01/02/06 |
